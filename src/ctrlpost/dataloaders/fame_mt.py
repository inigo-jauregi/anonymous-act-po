'''
Dataloaders for the FAME-MT dataset.
'''
import random
import re
...
class FameMtDataset(Dataset):
    def __init__(
        ...
        if og_data:
            # Load og data differrently (non-contrastive)
            df_formal = df.loc[df['formality'] == 'formal']
            df_informal = df.loc[df['formality'] == 'informal']
            df_neutral = df.loc[df['formality'] == 'neutral']
            source_texts_formal = df_formal['src'].tolist()
            source_texts_informal = df_informal['src'].tolist()
            source_texts_neutral = df_neutral['src'].tolist()
            target_texts_formal = df_formal['tgt'].tolist()
            target_texts_informal = df_informal['tgt'].tolist()
            target_texts_neutral = df_neutral['tgt'].tolist()
            target_texts_formal_annotated = df_formal['tgt'].tolist()
            target_texts_informal_annotated = df_informal['tgt'].tolist()
            target_texts_neutral_annotated = df_neutral['tgt'].tolist()
        else:
            # Load contrastive data
            # Combine into single list
            source_texts = df['src'].tolist()
            target_texts_formal = df['tgt_formal'].tolist()
            target_texts_informal = df['tgt_informal'].tolist()
            target_texts_formal_annotated = df['tgt_formal_annotated'].tolist()
            target_texts_informal_annotated = df['tgt_informal_annotated'].tolist()
        ...
        # Combine into single list
        if og_data:
            source_texts = source_texts_formal + source_texts_informal + source_texts_neutral + source_texts_neutral
            target_texts = target_texts_formal + target_texts_informal + target_texts_neutral + target_texts_neutral
            # Not annotated samples in original data, just a placeholder
            target_texts_annotated = target_texts_formal + target_texts_informal + target_texts_neutral + target_texts_neutral
            formality_labels = (['formal'] * len(target_texts_formal) + ['informal']*len(target_texts_informal) +
                                ['formal'] * len(target_texts_neutral) + ['informal'] * len(target_texts_neutral))
        else:
        ...
        if add_synthetic_pref_data:
            ...
            if only_synthetic:
                ...
                formality_labels = syn_contrastive_texts
            else:
                source_texts += syn_src_texts
                target_texts += syn_tgt_texts
                target_texts_annotated += len(syn_src_texts) * ['']
        ...
        if contrastive_dataset:
            if add_synthetic_pref_data:
                ...
                if og_data:
                    self.target_texts_contrastive = target_texts_formal + target_texts_informal + target_texts_neutral + target_texts_neutral
                    self.target_texts_annotated_contrastive = target_texts_formal + target_texts_informal + target_texts_neutral + target_texts_neutral
                else:
...
class FameMtDataModule(pl.LightningDataModule):
    def __init__(
            self,
            train_data_path: str,
            val_data_path: str,
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage in [None, 'fit']:
            self.train_dataset = FameMtDataset(
            ...
            self.val_dataset = FameMtDataset(
                self.val_data_path,
                self.src_lang,
                self.tgt_lang,
                self.tokenizer,
        ...
        if stage in [None, 'test']:
            ...
            if self.test_data_path:
                self.test_dataset = FameMtDataset(
                    self.test_data_path,
                    self.src_lang,
                    self.tgt_lang,
                    self.tokenizer,
